...
Then register the session in the tele dashboard (silent fail if tele not running):
```bash
...
5. Register the session in the tele dashboard (silent fail if tele not running):
   ```bash
   curl -s -X POST http://localhost:7373/api/threads/link \
     -H "Content-Type: application/json" \
     -d "{\"slug\": \"${SLUG}\", \"session_id\": \"${CLAUDE_SESSION_ID:-unknown}\", \"title\": \"${ARGUMENTS}\", \"cwd\": \"$(pwd)\"}" \
     > /dev/null 2>&1 || true
   ```
